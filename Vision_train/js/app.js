...
function computeRepScores(rep) {
    ...
    // We expect minExtension ~ 160–180, maxFlexion ~ 0–40
    let romScore = 1;
    // penalize if not extending enough
    if (minExtension < 150) {
        romScore *= clamp01((minExtension - 90) / 60); // 90–150 maps to 0–1
    }
    // penalize if not curling enough
    if (maxFlexion > 50) {
        romScore *= clamp01((160 - maxFlexion) / 110); // 50–160 → 1–0 approx
    }
