    ...
    // For crude momentum detection (hip vertical movement)
    ...
    function computeRepScores(rep) {
        ...
        // 1) Time factor (2–6 seconds best for squats)
        let timeScore;
        if (repDuration < 2) {
        ...
        let romScore = 1;
        if (minKneeExtension < 150) {
            romScore *= clamp01((minKneeExtension - 90) / 60);
        }
        if (maxKneeFlexion > 100) {
            romScore *= clamp01((90 - maxKneeFlexion + 170) / 80);
        }
        ...
        // 4) Momentum factor
        let momentumScore;
        if (avgHipSpeed <= 0.006) {
            momentumScore = 1;
        } else if (avgHipSpeed >= 0.025) {
            momentumScore = 0;
        } else {
            momentumScore = clamp01(1 - (avgHipSpeed - 0.006) / 0.019);
        }
    ...
    function analyzeSquatForm(kneeAngle, backAngle, leftKnee, rightKnee, leftHip, rightHip, leftShoulder, rightShoulder, landmarks) {
        ...
        // Calculate knee angles locally for symmetry
        ...
        // FIXED Back angle check (0-30° range, realistic for upright posture)
        if (backAngle > 25) {
    ...
    function onResults(results) {
        ...
        if (results.poseLandmarks) {
            ...
            // FIXED Back angle - torso lean from vertical (0°=perfectly upright)
            const torsoHeight = Math.abs(leftShoulder.y - leftHip.y);
            const torsoLeanX = Math.abs(leftShoulder.x - leftHip.x);
            const backAngle = Math.min(90, (torsoLeanX / torsoHeight) * 90); // 0-90° range
            ...
            // Momentum: hip vertical movement
            if (lastLeftHipY !== null && lastRightHipY !== null) {
                ...
                if (hipVelocityBuffer.length > 30) {
            ...
            // Rep counting (kneeAngle < 95° = squat detected)
            if (kneeAngle > 165) {
                if (stage === 'down') {
                    stage = 'up';
                    repCount++;
            ...
            } else if (kneeAngle < 95) { // SQUAT DETECTED AT <95°
                if (stage === 'up') {
                    stage = 'down';
            ...
            // FIXED function call - pass landmarks for symmetry calculation
